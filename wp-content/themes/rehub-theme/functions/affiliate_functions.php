...
function rh_review_frontend_fields($current_values, $form_id){
	...
	if (!empty($reviewCriteria[$form_id])){
		...
		$criteriaInputs .= '<div class="your_total_score">'.__('Tổng điểm của bạn','rehub-theme').' <span class="user_reviews_view_score"><span class="userstar-rating"><span style="width: '.$currentReviewscore.'%"></span></span></span></div><input type="hidden" name="criteria_names" value="'.implode(",", $criteriaNamesArray).'" />';
	    ...
	    <div id="user_reviews_in_frontend" class="rate_bar_wrap pt10 mt30">
	        ...
	        <div class="wpfepp-form-field-container">
				<label><?php esc_html_e('Ưu điểm. Add each from separate line', 'rehub-theme');?></label>
	        	<textarea name="review_post_pros_text"><?php echo ''.$review_proses; ?></textarea>
	        </div>
	        <div class="wpfepp-form-field-container">
				<label><?php esc_html_e('Nhược điểm. Add each from separate line', 'rehub-theme');?></label>
